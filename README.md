...
.github/workflows/discord-notify.yml  # 毎週月曜に latest.json を送信
logs/                              # 実行ログ（git管理外）
```
...
```sh
# 1. 送信先の Discord Webhook を登録（GitHub Actions が使う）
...
# 4. 送信を手動で試す（フラグを消したくない場合は clear_new=false）
gh workflow run Discord通知 -f clear_new=false
gh run watch
```

**GitHub Actions 側に Claude の認証情報は不要です。** 送信スクリプトはPython標準ライブラリだけで動きます。
...
- scheduled workflow は**リポジトリに60日間活動がないと自動停止**します。ローカルの日次調査でコミットが入るため通常は問題ありません
- ローカルの調査が数日止まっても、通知自体は GitHub 側で必ず出ます（内容が古いままになるだけ）
